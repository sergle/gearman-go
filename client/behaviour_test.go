...
//
// Flaky here under -race (~84 failed assertions per 500 runs) until Close got
// its closed flag: readLoop redialed after the socket went away and the
// resurrected connection answered calls that must fail. 1500 runs clean
// since. If it reappears, find the new route in; do not relax the assertion.
